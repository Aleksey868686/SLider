...
namespace LifeSpot
{
    public static class EndpointMapper
    {
        ...
        public static void MapImages(this IEndpointRouteBuilder buider)
        {
            var imageFiles = new[] { "london.jpg", "ny.jpg", "spb.jpg" };
            foreach (var image in imageFiles)
            {
                buider.MapGet($"/Static/images/{image}", async context =>
                {
                    var imagePath = Path.Combine(Directory.GetCurrentDirectory(), "Static", "images", image);
                    var bytes = await File.ReadAllBytesAsync(imagePath);
                    context.Response.ContentType = "image/jpeg";
                    await context.Response.Body.WriteAsync(bytes, 0, bytes.Length);
                    //var jpg = await File.ReadAllTextAsync(imagePath);
